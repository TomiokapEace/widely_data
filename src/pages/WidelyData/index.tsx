...
import {WidelyDataDelete, WidelyDataGetDataSelect, WidelyDataGetData,WidelyDataSave, WidelyDataUpdate} from "./WidelyDataController";
    ...
    const actionRef = useRef<ActionType>();
        const [layoutType, setLayoutType] = useState<ProFormLayoutType>('DrawerForm');
            ...
            <ProTable<GithubIssueItem>
                ...
                // @ts-ignore
                const result=await WidelyDataGetDataSelect(params).then(res =>{
                    const result = {
                ...
                return WidelyDataUpdate(record).then((resolve)=>{
                if(resolve.code==200){
                ...
                onDelete:(key, row)=>{
                return WidelyDataDelete(row.id).then((resolve)=>{
                    if(resolve.code==200){
                ...
                <BetaSchemaForm<GithubIssueItem>
                    ...
                    console.log(values)
                    return  WidelyDataSave(values).then((resolve)=>{
                    if(resolve.code==200){
